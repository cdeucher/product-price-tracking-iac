...
resource "aws_iam_policy" "dynamodb_access" {
  count       = length(concat(var.dynamodb_arn, var.dynamodb_stream_arn)) > 0 ? 1 : 0
  ...
  policy = templatefile("${path.module}/policies/dynamodb_policy.json", {
    arns = concat(var.dynamodb_arn, var.dynamodb_stream_arn)
  })
}

resource "aws_iam_policy" "sqs_access" {
  count       = length(concat(var.sqs_arn, var.sqs_stream_arn)) > 0 ? 1 : 0
  name        = "${var.project}-${var.function_name}-sqs-access"
